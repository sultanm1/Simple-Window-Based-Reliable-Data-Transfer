...
// FILE* writeTOFILE;
// char myname[5] = "OKIE";
// writeTOFILE = fopen(myname, "w");
// int ackNum = (synackpkt.seqnum + 1) % MAX_SEQN;
//
//
// fwrite(buf, 1, m, writeTOFILE);
...
// printf("Buffer is\n");
// printf("%s\n", buf);
// buildPkt(&pkts[1], seqNum, ackNum, 0, 0, 0, 0, m, buf);
// printSend(&pkts[1], 0);
// printf("m value is %d\n", (int)m);
// sendto(sockfd, &pkts[1], sizeof(buf), 0, (struct sockaddr*) &servaddr, servaddrlen);
// fwrite(buf, 1, m, writeTOFILE);
// seqNum += (int)m;
// memset(buf, 0, sizeof(buf));
...
//   }
//
//   // printf("Buffer from client is \n");
//   // printf("%s", buf);
//
//
//   buildPkt(&pkts[i], seqNum, ackNum, 0, 0, 0, 0, m, buf);
//   printSend(&pkts[i], 0);
//   printf("m value is %d\n", m);
//   sendto(sockfd, &pkts[i], sizeof(buf), 0, (struct sockaddr*) &servaddr, servaddrlen);
//   seqNum += (int)m;
//   lessThanWindowSize++;
// }
// seqNum += (int)m;
...
// m = fread(buf, 1, PAYLOAD_SIZE, fp);
// buildPkt(&pkts[2], seqNum, ackNum, 0, 0, 0, 0, m, buf);
// printSend(&pkts[2], 0);
// printf("m value is %d\n", (int)m);
// sendto(sockfd, &pkts[2], sizeof(buf), 0, (struct sockaddr*) &servaddr, servaddrlen);
// fwrite(buf, 1, m, writeTOFILE);
//
// seqNum += (int)m;
